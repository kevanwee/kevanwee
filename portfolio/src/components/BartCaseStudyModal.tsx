...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    body: "MatchingService is the retrieval abstraction layer, wiring four infrastructure components behind a clean interface: a Qwen3-0.6B dense embedder, a Chroma/pgvector vector store, a BM25Okapi sparse index with a legal-aware tokeniser, and a BGE cross-encoder reranker. Swapping backends (e.g. Chroma to RDS pgvector for production) requires only a config flag.",
    images: [
      {
  ...
  },
  {
    ...
    body: "Offline ingestion chunks the statute corpus (800 chars per chunk max), embeds with Qwen3-0.6B, and populates both the vector store and the BM25 index. Each chunk carries version metadata (YYYYMMDD) so the verification step can flag outdated, removed, or renumbered provisions.",
    images: [
      {
        src: "/projects/bart/image21.png",
        caption: "Persistent stores: Statute Corpus (JSON, version-keyed) ingested into Vector Store (Chroma/pgvector, 1024-dim cosine) and BM25 Index (~299 MB, legal-aware tokeniser). Scan results are transient and discarded on server restart.",
        wide: true,
      },
    ],
  },
    ...
    images: [
      {
        ...
        caption: "Word add-in task pane showing match results with confidence scores, verification status (VALID / AMENDED / REMOVED / RENUMBERED), and inline suggested corrections.",
      },
      {
        src: "/projects/bart/image3.png",
        caption: "Citation detection in action: statute references extracted from the document, matched against the corpus, and surfaced with evidence excerpts and recommended actions.",
      },
    ],
  },
  {
    ...
    images: [
      {
        ...
        caption: "CI pipeline architecture: lint gate (ruff, py310/py312) unblocking parallel scraper-tests (pytest, JUnit XML), backend-tests (pytest + coverage), and frontend-tests (Vitest, 6 files, 65 tests).",
        wide: true,
      },
      {
